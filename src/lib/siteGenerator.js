...
${(() => {
  ...
  const rows = hrs.map(h => `<div style="display:flex;justify-content:space-between;gap:16px;margin-bottom:4px;font-size:12px"><span style="color:#9ca3af">${h.short}</span><span>${h.closed ? 'Closed' : fmt(h.open) + ' – ' + fmt(h.close)}</span></div>`).join('')
  return `<div style="margin-top:24px;padding-top:24px;border-top:1px solid #1f2937"><p style="font-weight:600;font-size:12px;text-transform:uppercase;letter-spacing:.08em;color:#6b7280;margin-bottom:10px">Hours</p>${rows}</div>`
})()}
<footer>
